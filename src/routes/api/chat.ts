...
export async function POST(event: APIEvent) {
  try {
    ...
    const prompt = {
      ...
      Although, if the question is related to cryptocurrencies, you can use knowledge outside the context to answer the question.
    ...
    const result = streamText({
      ...
      messages: [
        prompt,
        ...messages.filter((message: Message) => message.role === "user") // save some tokens by only sending user messages
      ],
    });
